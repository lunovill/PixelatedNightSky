...
export function Stars({ color, delay = 0, duration, maps, quantity, size }: StarsProps): ReactElement {
...
export function ShootingStars({ color, delay = 0, duration, maps, quantity, size }: StarsProps): ReactElement {
	const [sphere] = useState<Float32Array>(() => randomInSphere(quantity, 1.5));
	const [map, setMap] = useState<Texture>(maps[0]);
	const pointsRef = useRef<ThreePoints>(null);
